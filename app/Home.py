...
from agents.combat import get_combat_agent
from agents.example import get_example_agent

nest_asyncio.apply()
st.set_page_config(
    page_title="CombatManager",
    page_icon=":orange_heart:",
)
st.title("CombatManager")
...
def main() -> None:
    ...
    # Get Model Id
    model_id = st.sidebar.selectbox("Model", options=["gpt-4o-mini", "gpt-4o"])
    ...
    if "example_agent" not in st.session_state or st.session_state["example_agent"] is None:
        ...
        example_agent = get_combat_agent(model_id=model_id, debug_mode=True)
    ...
    if len(agent_chat_history) > 0:
        ...
        st.session_state["messages"] = [{"role": "assistant", "content": "Ask me to start the combat scenario..."}]
    ...
    if example_agent.storage:
        ...
        if st.session_state["example_agent_session_id"] != new_example_agent_session_id:
            ...
            st.session_state["example_agent"] = get_combat_agent(
